...
export async function getClosuresTool(args) {
    ...
    const closures = allClosures.filter((c) => c.name.toLowerCase().includes(hawkerName.toLowerCase()));
    ...
    const responseText = `Closure information for "${hawkerName}":\n\n` +
        closures
            .map((c) => `• ${c.quarter}: ${c.closureDates}\n  Reason: ${c.reason}`)
            .join('\n\n');
    return {
        content: [
            {
