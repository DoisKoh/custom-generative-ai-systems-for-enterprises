...
export async function getHawkerDetailsTool(args) {
    ...
    const hawker = allHawkers.find((h) => h.name.toLowerCase() === hawkerName.toLowerCase()) || allHawkers.find((h) => h.name.toLowerCase().includes(hawkerName.toLowerCase()));
    if (!hawker) {
    ...
    const responseText = `Details for ${hawker.name}:\n\n` +
