...
export async function fetchHawkerCentres(): Promise<HawkerCentre[]> {
  ...
  try {
    ...
    const hawkers: HawkerCentre[] = geojson.features.map((feature: any) => ({
...
let cachedClosures: HawkerClosure[] | null = null;
let closureCacheTimestamp: number = 0;
...
export async function fetchHawkerClosures(): Promise<HawkerClosure[]> {
  ...
  try {
    ...
    const closures = data.result.records.map((record: any) => ({
      name: record.name || '',
